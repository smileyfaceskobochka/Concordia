...
void Texture::load(const std::string &path, Allocator &allocator,
  ...
  allocator.createImage(
      m_width, m_height, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL,
  ...
  allocator.transitionImageLayout(
      ...
      VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, graphicsQueue, cmdPool, device);

  // Copy the staging buffer to the image
  allocator.copyBufferToImage(stagingBuffer, m_image, m_width, m_height,
                              graphicsQueue, cmdPool, device);

  // Transition to shader read
  allocator.transitionImageLayout(
      m_image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
      VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, graphicsQueue, cmdPool, device);
...
void Texture::destroy(Allocator &allocator, VkDevice device) {
  ...
  if (m_image)
    vmaDestroyImage(allocator.getVma(), m_image, m_allocation);
