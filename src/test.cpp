// Точно работает (Своими глазвми видел)
...
#define VK_CHECK_LOG(fn)                                                       \
  do {                                                                         \
    VkResult res = (fn);                                                       \
    if (res != VK_SUCCESS) {                                                   \
      std::cerr << "[VK_ERROR] " #fn " -> " << res << " (" << __FILE__ << ":"  \
                << __LINE__ << ")" << std::endl;                               \
...
private:
  ...
  void initInstance() {
    ...
    std::cout << "[Init] Vulkan extensions count=" << instExts.size()
              << std::endl;
  ...
  void createSwapchain() {
    ...
    uint32_t fmtCount = 0;
    VK_CHECK_LOG(vkGetPhysicalDeviceSurfaceFormatsKHR(physDevice, surface,
                                                      &fmtCount, nullptr));
    ...
    extent = (caps.currentExtent.width == UINT32_MAX) ? VkExtent2D{800, 600}
                                                      : caps.currentExtent;
  ...
  void createGraphicsPipeline() {
    ...
    gpci.subpass = 0;
    VK_CHECK_LOG(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &gpci,
                                           nullptr, &pipeline));
  ...
  void recordCommandBuffers() {
    ...
    for (size_t i = 0; i < cmdBuffers.size(); ++i) {
      ...
      vkCmdBindPipeline(cmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                        pipeline);
